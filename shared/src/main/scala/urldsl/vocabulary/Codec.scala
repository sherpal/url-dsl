...
object Codec:
  ...
  given [Left, Middle, Right](
      using
      leftCodec: Codec[Left, Middle],
      rightCodec: Codec[Middle, Right]
  ) as Codec[Left, Right] = leftCodec ++ rightCodec

  given [Left, Right](using codec: Codec[Left, Right]) as Codec[List[Left], List[Right]] =
    factory(_.map(codec.leftToRight), _.map(codec.rightToLeft))

  given [T, U] as Codec[(List[T], List[U]), List[(T, U)]] = factory(
