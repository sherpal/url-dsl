...
trait PathSegment[T, A] {
  ...
  /**
    ...
    * rules.
    */
  @alpha("then") final def /[U](that: PathSegment[U, A])(implicit ev: Tupler[T, U]): PathSegment[ev.Out, A] =
  ...
  @alpha("withQuery") final def ?[ParamsType, QPError](
  ...
  /**
    ...
    */
  @alpha("or") final def ||[U](that: PathSegment[U, A]): PathSegment[Either[T, U], A] = PathSegment.factory[Either[T, U], A](
    segments =>
  ...
  /**
    ...
    */
  final def as[U](using codec: Codec[T, U]): PathSegment[U, A] = as[U](codec.leftToRight, codec.rightToLeft)
  ...
  final def as[U](fromTToU: T => U, fromUToT: U => T): PathSegment[U, A] = PathSegment.factory[U, A](
    (matchSegments).andThen(_.map(_.map(fromTToU))),
...
object PathSegment {
  ...
  ): PathSegment[T, A] = simplePathSegment[T, A]((fromString.apply).compose(_.content), printer.print)
