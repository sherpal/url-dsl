...
trait PathSegment[T, +A] extends UrlPart[T, A]:
  ...
  /**
    * Sugar when `T =:= Unit`
    */
  final def createSegments()(using ev: Unit =:= T): List[Segment] = createSegments(ev(()))
  ...
  /**
    * Sugar when `T =:= Unit`
    */
  final def createPath()(using ev: Unit =:= T): String =
    createPath(ev(()))
  final def createPath(encoder: UrlStringGenerator)(using ev: Unit =:= T): String =
    createPath(ev(()), encoder)
  ...
  final def filter(predicate: T => Boolean)(using ev: A <:< DummyError): PathSegment[T, DummyError] =
  ...
  /**
    ...
    */
  final def as[U](using codec: Codec[T, U]): PathSegment[U, A] = as[U](codec.leftToRight _, codec.rightToLeft _)
  ...
  final def provide[A1 >: A](
      t: T
  )(using pathMatchingError: PathMatchingError[A1], printer: Printer[T]): PathSegment[Unit, A1] =
    PathSegment.factory[Unit, A1](
      segments =>
        for
          tMatch <- matchSegments(segments)
          PathMatchOutput(tOutput, unusedSegments) = tMatch
          unitMatched <- if tOutput != t then Left(pathMatchingError.wrongValue(printer(t), printer(tOutput)))
...
object PathSegment:
  ...
  final def noMatch[A](using pathMatchingError: PathMatchingError[A]): PathSegment[Unit, A] =
  ...
  final def simplePathSegment[T, A](matching: Segment => Either[A, T], creating: T => Segment)(
      using pathMatchingError: PathMatchingError[A]
  ): PathSegment[T, A] =
    factory(
      (_: Seq[Segment]) match {
  ...
  final def stringSegment[A](using pathMatchingError: PathMatchingError[A]): PathSegment[String, A] =
    segment[String, A]
  ...
  /**
    ...
    */
  final def endOfSegments[A](using pathMatchingError: PathMatchingError[A]): PathSegment[Unit, A] = factory[Unit, A](
